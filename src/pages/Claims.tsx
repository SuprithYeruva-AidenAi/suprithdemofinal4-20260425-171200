...
export default function Claims() {
  ...
  async function _submitClaim(opts: { claimType: string; policyId: string; description?: string }) {
    ...
    try {
      ...
      await uploadDocument(_uploadFile, { claimType: opts.claimType, policyId: opts.policyId });
      await sendEmail({
        to: 'claims@uoi.com.sg',
        subject: `New ${opts.claimType} claim for policy ${opts.policyId}`,
        body: opts.description || `New ${opts.claimType} claim for policy ${opts.policyId}.`,
      });
      setSubmitMsg({ kind: 'ok', text: 'Claim submitted successfully.' });
    } catch (e: any) {
